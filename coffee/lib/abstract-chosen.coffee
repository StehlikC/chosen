          ...
          if option.search_match
            if searchText.length
              if @search_fuzzy
                ...
                tokens = searchText.split(' ').sort (a, b) ->
                  b.length - a.length

                search_string = option.search_text.split ' '

                for string in search_string
                  for token in tokens
                    if string.indexOf('<em>') == -1 && string.indexOf('</em>') == -1
                      string = string.replace(new RegExp('(' + token + ')', 'gi'), '<em>$1</em>')
                  search_text.push string

                option.search_text = search_text.join ' '
              else
